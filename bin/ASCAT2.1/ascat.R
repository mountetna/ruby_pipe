...
# stuff you need for sure:
## Tumor_LogR
## samples
## Tumor_BAF
# Tumor_LogR_segmented
# Tumor_BAF_segmented
# ch, goddammit
...
runASCAT = function(lrr, baf, lrrsegmented, bafsegmented, chromosomes, distancepng = NA, copynumberprofilespng = NA, nonroundedprofilepng = NA, gamma = 0.55, allow100percent=T) {
  ch = chromosomes
  b = bafsegmented
  ...
  for (i in 4:(dim(d)[1]-3)) {
	...
	#print.noquote(paste("Ploidy is ",ploidy,"rho is", rho, "goodnessofFit is", goodnessOfFit, "PZ is", percentzero, "pzabb is", perczeroAbb ))

        if (ploidy > 1.5 & ploidy < 5.5 & rho >= 0.2 & goodnessOfFit > 70 & (percentzero > 0.01 | perczeroAbb > 0.1)) {
	  #print.noquote("Looks good, adding opt.")
          nropt = nropt + 1
