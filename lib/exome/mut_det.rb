    ...
    class FilterMuts
      ...
      def run
        File.open(config.tumor_maf, "w") do |f|
          f.puts [ :gene, :chrom, :pos, :ref_allele, :alt_allele, :tumor_ref_count, :tumor_alt_count, :tumor_var_freq, :normal_ref_count, :normal_alt_count, :variant_classification, :protein_change, :class ].join("\t")
          config.sample.chroms.each do |chrom|
            ...
            m.each do |l|
              ...
              f.puts [ l.onco.txp_gene, l.contig, l.position, l.ref_allele, l.alt_allele, 
                l.t_ref_count, l.t_alt_count, l.t_var_freq, l.n_ref_count, l.n_alt_count, 
